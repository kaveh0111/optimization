...
\label{tab:ogan_inputs}
\renewcommand{\arraystretch}{1.5} % Adjust this value to increase the height of all rows
\begin{tabular}{|c|l|}
\hline
\multicolumn{2}{|c|}{\textbf{Input parameters}} \\
\hline
$\eta_i^{(g)}$ & GPU cycles consumed per iteration of $\mathrm{GAN}_i$ \\
\hline
$\eta_i^{(c)}$ & CPU cycles consumed per DRL training step $i$ \\
\hline
$E_i^{\max}$ & Upper tolerance on surrogate NND fidelity for $\mathrm{GAN}_i$ \\
\hline
$\Psi_i$ & Maximum CPU cores usable for $\mathrm{DRL}_i$ at the MEC \\
\hline
$T_i^{\max}$ & Remaining time until the wireless episode terminates \\
\hline
$\Phi_i$ & Anticipated DRL training steps scheduled for iteration $i$ \\
\hline
$B^{\mathrm{bh}}$ & Aggregate backhaul spectral budget pooled across selective uploads \eqref{eq:bw_all} \\
\hline
$B_i$ & Dedicated bandwidth (Hz) contracted for uplink \(i\to\mathrm{MBS}\) when active \\
\hline
$R_i$ & \(\mathrm{SBS}_i\to\mathrm{MBS}\) attainable rate \(B_i\log_2(1+\mathrm{SNR}_i)\), bit/s \\
\hline
$R^{\mathrm{cloud}}$ & \(\mathrm{MBS}\to\)cloud egress budget aggregated in \eqref{eq:R_all} \\
\hline
$R_i^{\mathrm{dc}}$ & Goodput attributable to \(\Upsilon_i\) traversing \(\mathrm{MBS}\to\)cloud conduit \\
\hline
$N_{\Upsilon_i}$, $\bar{V}_i$, $\bar{W}_i$ & Rare-sample cardinality and calibrated Frobenius aggregates from Tab.~\ref{tab:offline_training} \\
\hline
$\lambda_t$, $\lambda_e$ & Weights regulating latency vs fidelity in (\ref{eq:obj}) \\
\hline
$c^{\min}$ & Minimum nonzero CPU allotment when \(z_i=1\) \\
\hline
$m_i^{\max}$ & Architectural cap on iterative GAN depth per episode \\
\hline
$N_g$, $N_c$, $N$ & Available GPUs (cloud); CPU cores at MEC; total SBSs \\
\hline
$F_g$, $F_c$ & GPU/CPU throughput scaling factors (effective cycles\,s\(^{-1}\)) \\
\hline
\end{tabular}
\end{table}
...
Objective (\ref{eq:obj}) penalises remaining idle with a unit miss term \(1-z_i\). When \(z_i=1\), it aggregates episode-normalised latency \(\lambda_t(T_i^{com}+m_i\eta_i^{(g)}/F_g+\Phi_i\eta_i^{(c)}/(F_cc_i))/T_i^{\max}\) and normalised surrogate fidelity \(\lambda_e(\bar{V}_i/\sqrt{m_i}+\bar{W}_i/\sqrt{N_{\Upsilon_i}})/E_i^{\max}\) from (\ref{eq:gan_error}); \(\lambda_t\) and \(\lambda_e\) keep seconds and unitless error on comparable scales (often \(\lambda_t+\lambda_e=1\)). Larger \(\tilde{\beta}_i\) drives resource toward SBSs with higher empirical loss ratios \(\beta_i=\nu_i^l/\nu_i^t\) after normalisation.
